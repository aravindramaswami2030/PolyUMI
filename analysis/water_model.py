...
class WaterClassifier(nn.Module):
    ...
    def __init__(self, modalities: tuple[str, ...], n_classes: int = 3,
                 d_embed: int = 128, hidden: int = 256, dropout: float = 0.5):
        ...
        self.encoders = nn.ModuleDict({
            m: (AudioEncoder(d_embed) if m == 'audio' else ImageEncoder(d_embed))
