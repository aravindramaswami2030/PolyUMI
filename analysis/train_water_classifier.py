...
Usage (on lamb, with ROS sourced so sklearn is importable):
...
def build_model(kind: str, seed: int = 0):
    ...
    if kind == 'logreg':
        ...
        head = MLPClassifier(hidden_layer_sizes=(64,), alpha=1.0, max_iter=5000,
                             random_state=seed)
    else:
...
def main() -> int:
    ...
    ap.add_argument('--n-permutations', type=int, default=30,
                    help='shuffled-label runs used to find the real chance floor')
    ...
    for _ in range(args.n_permutations):
        ...
        pred = cross_val_predict(build_model(args.models[0], args.seed), X_all, y_shuf,
                                 cv=LeaveOneOut())
        null.append(float((pred == y_shuf).mean()))
    null = np.asarray(null)
    thresh = float(np.percentile(null, 95))
    print(f'  {args.n_permutations} shuffles: mean {null.mean():.3f}  p95 {thresh:.3f}  '
          f'max {null.max():.3f}')
    ...
    print(f'vision-only baseline (best model): {vision_only:.3f}  '
